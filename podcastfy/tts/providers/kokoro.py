...
class KokoroTTS(TTSProvider):
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "kokoro"):
        """
        Initialize Kokoro TTS provider.
        
        Args:
            api_key: Not used, kept for compatibility
            model: Model name to use. Defaults to "kokoro"
        """
        self.base_url = "http://localhost:8880/v1"
        self.model = model
            
    def generate_audio(self, text: str, voice: str, model: str, voice2: str = None) -> bytes:
        """Generate audio using Kokoro API with support for two voices."""
        ...
        try:
            ...
            for match in matches:
                if match.group(1) is not None:  # Person1
                    ...
                    if text_part:
                        parts.append((text_part, voice2 or "af_bella"))
